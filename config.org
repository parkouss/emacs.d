...
#+BEGIN_SRC emacs-lisp
    (use-package multi-term
      ...
      ;; the original list, but without the ESC key since I never use it
      ;; in emacs (meaning that it will be sent to the terminal)
...
#+BEGIN_SRC emacs-lisp
  (use-package irony
    ...
    (add-hook 'c-mode-hook 'irony-mode)
    ;; irony does not find the database without that...
...
#+BEGIN_SRC emacs-lisp
  (use-package jabber
    ...
    :init
    (setq jabber-account-list (if has-secrets
                                  ;; steery.io account
